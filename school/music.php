...
{
    $id = $_GET['delete'];
    $stm = "DELETE FROM music WHERE Id=$id";
...
if(isset($_POST['update']))
{
    ...
    $stm = "UPDATE music SET Name='$name',Path='$pname',Type='$filetype' WHERE Id=$id";
    mysqli_query($conn,$stm)or die($mysqli->error());
}
...
if(isset($_GET['edit']))
{
    $id = $_GET['edit'];
    $update = true;
    $stm = "SELECT * FROM music WHERE Id=$id";
...
if(isset($_POST['submit']))
    {
        ...
        $stm = "INSERT INTO music (Name,Path,Type) VALUES ('$name','$pname','$filetype');";
        try{
           ...
           while( $category = mysqli_fetch_array($data))
           {
               ?>
                <tr>
                    ...
                    <td>
                        <a style="color: white;margin-right: 1rem;border: 1px;border-radius: 100%;padding: .9rem;" href="music.php?edit=<?php echo $category['Id']; ?>" >Edit</a>
                        <a style="color: white;margin-right: 1rem;border: 1px;border-radius: 100%;padding: .9rem .3rem;" href="music.php?delete=<?php echo $category['Id']; ?>" >Delete</a>
                    </td>
                </tr>
...
<?php
           ...
           <form style="margin: 4rem 0 0 32rem;" action="music.php" method="POST" enctype="multipart/form-data">
